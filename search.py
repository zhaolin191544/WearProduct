...
def out_to_mean_x_range(out_min: float, out_max: float, target_low: float, target_high: float, eps: float = 1e-12) -> Tuple[float, float]:
    """
    Interpret target_low/target_high as the final expected output range already.
    We no longer remap them via out_min/out_max; just clamp into [0,1].
    """
...
def search_bucket_all_plans_with_crate_ratio1(
    materials: List[Material],
    slot_ranges: List[dict],
    L_all: float,
    U_all: float,
